...
namespace PortableStorage.UI
{
	...
	public class UIDevNullSlot : UIElement
	{
		...
		public override void Click(UIMouseEvent evt)
		{
			...
			else
			{
				if (Main.mouseItem.IsAir)
				{
					...
					if (Item.stack <= 0)
					{
						Item.TurnToAir();
						active = false;
						devNullUI.devNull.selectedItem = null;
					}
				}
			...
			NetHelper.SyncEntity(MessageID.SyncItem, slot);
		}
		...
		//			devNullUI.devNull.selectedItem = Item;
		//			active = true;
		//		}
		//		else
		//		{
		...
		// alt+click - select for placement
		// click - if already existing -> add to it
		//		   else add it to the slot
		// placing won't extract the last item
		...
		protected override void DrawSelf(SpriteBatch spriteBatch)
		{
			...
			if (!Item.IsAir)
			{
				...
				if (Item.stack > 1) ChatManager.DrawColorCodedStringWithShadow(spriteBatch, Main.fontItemStack, Item.stack.ToSI("F1").TrimEnd('.', '0'), dimensions.Position() + new Vector2(10f, 26f) * scale, Color.White, 0f, Vector2.Zero, new Vector2(scale), -1f, scale);

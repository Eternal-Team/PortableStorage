...
namespace PortableStorage.UI
{
    public class AmmoBeltUI : BaseUI
    {
        ...
        private void LootAllClick(UIMouseEvent evt, UIElement listeningElement)
        {
            ...
            {
                TheOneLibrary.Utility.Utility.LootAll(ammoBelt);
                Recipe.FindRecipes();
            }
        }
        ...
        private void DepositAllClick(UIMouseEvent evt, UIElement listeningElement)
        {
            if (Main.player[Main.myPlayer].chest == -1 && Main.npcShop == 0)
            {
                TheOneLibrary.Utility.Utility.DepositAll(ammoBelt, item => item.ammo > 0);
                Recipe.FindRecipes();
            }
        }
        ...
        private void QuickRestack(UIMouseEvent evt, UIElement listeningElement)
        {
            if (Main.player[Main.myPlayer].chest == -1 && Main.npcShop == 0)
            {
                TheOneLibrary.Utility.Utility.QuickRestack(ammoBelt);
                Recipe.FindRecipes();
            }
        }
        ...
        private void QuickStackClick(UIMouseEvent evt, UIElement listeningElement)
        {
            if (Main.player[Main.myPlayer].chest == -1 && Main.npcShop == 0)
            {
                TheOneLibrary.Utility.Utility.QuickStack(ammoBelt);
                Recipe.FindRecipes();
            }
        }
